...
namespace Editor::Layers {
    ...
    //        // Initialize text buffers once per open
    //        if (!s.initialized) {
    ...
    //        // Duplicate detection (trim + case-insensitive)
    //        {
    //            s.dup.fill(false);
    ...
    //        if (ImGui::BeginTable("##LayerEditTable", 3,
    //            ImGuiTableFlags_RowBg |
    //            ImGuiTableFlags_BordersInnerV |
    //            ImGuiTableFlags_SizingFixedFit)) {
    //            ImGui::TableSetupColumn("ID", ImGuiTableColumnFlags_WidthFixed, 40.0f);
    //            ImGui::TableSetupColumn("Used", ImGuiTableColumnFlags_WidthFixed, 42.0f);
    ...
    //                bool used = db.IsUsed(id); // depends on your definition (used && name not empty)
    //                // We'll display "used" based on whether the buffer is empty, not DB, for live feedback:
    //                {
    //                    std::string_view raw = TrimSV(s.buf[i].data());
    //                    used = !raw.empty();
    //                }
    //                ImGui::TextUnformatted(used ? "Yes" : "No");
    ...
    //                    ImGui::PopStyleColor();
    //                    ImGui::SameLine();
    //                    ImGui::TextColored(ImVec4(1, 0.4f, 0.4f, 1), "dup");
    //                }
    ...
    //        } else {
    //            ImGui::TextDisabled("Tip: empty name deactivates a layer.");
    //        }
    ...
    //            // - For each layer:
    //            //    * if reserved: skip
    //            //    * if trimmed name empty: RemoveLayer(id)
    //            //    * else:
    //            //        - if layer was inactive: CreateLayer(name) BUT we need specific slot id...
    //            //          Your CreateLayer finds a free slot, so we can't force "id".
    //            //          Therefore: we treat naming an empty slot as "RenameLayer(id,name)".
    //            //          RenameLayer should set used=true internally if needed.
    //            //
    //            // So we rely on RenameLayer(id,name) to activate slot and update maps.
    //            //
    //            bool ok = true;
    ...
    //                if (newName.empty()) {
    //                    // deactivate
    //                    // If your RemoveLayer() refuses for "unused" slots, that's fine.
    //                    // Just ignore false if you want. Here we treat failure as non-fatal.
    //                    db.RemoveLayer(id);
    //                } else {
    ...
    //                            "Failed to apply layer %d rename (invalid name?).", i);
    //                        break;
    //                    }
    //                }
    //            }
    ...
    //                s.initialized = false; // refresh from DB on next open
    //            }
    //        }
    ...
    //            ImGui::CloseCurrentPopup();
    //            s.initialized = false; // discard edits
    //        }

    //        ImGui::EndPopup();
    //    } else {
    //        // not open this frame -> reset init
    //        s.initialized = false;
    //    }
