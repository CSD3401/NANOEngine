...
namespace Editor {
	...
	CreateUIButtonEntityCommand::CreateUIButtonEntityCommand(uint32_t parentCanvas)
		: m_entity(0), m_parentCanvas(parentCanvas) {
	}

	void CreateUIButtonEntityCommand::Execute()
	{
		m_entity = NE::ECS::Command::CreateUIButtonEntity(m_parentCanvas);
		EditorScene::s_entities.push_back(EditorEntity{ m_entity });
		Editor::EditorScene::s_selectedEntity = &EditorScene::s_entities.back();
		...
		Editor::Node node{};
		node.id = m_entity;
		node.parent = m_parentCanvas;  // button belongs to canvas
	...
	void CreateUIButtonEntityCommand::Undo()
	{
		const uint32_t id = m_entity;
