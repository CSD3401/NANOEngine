...
namespace NE::ECS::Systems {
   ...
   //     }
    ...
    bool RenderSystem::TestSphereFrustum(const Frustum& F, const Mat4& M, float radiusLS) {
    ...
    void RenderSystem::FrustumCulling() {
        ...
        for (Entity e : entities)
        {
            ...
            if (m_componentManager->HasComponent<NE::ECS::Component::Collider>(e))
            {
                ...
                {
                    isVisible = TestSphereFrustum(frustum, modelMatrix, collider.radius);
                }
                break;
                ...
                case NE::ECS::Component::Collider::ShapeType::Box:
                {
                    ...
                    isVisible = TestAABBFrustum(frustum, modelMatrix, minLS, maxLS);
                }
                break;

                default:
                {
                    isVisible = TestSphereFrustum(frustum, modelMatrix, 0.5f); // default
                }
                break;
                }
            }
            ...
            if (!hasCollider)
            {
                if (renderer.model && renderer.model->hasSphereBoundsLS)
                {
                    ...
                    isVisible = TestSphereFrustum(frustum, modelMatrix, centerLS, radiusLS);
                }
                else
                {
                    isVisible = TestSphereFrustum(frustum, modelMatrix, 0.5f); // default
