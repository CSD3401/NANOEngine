...
namespace NE::Graphics {
    ...
    struct PipelineKey {
        ...
        inline uint64_t Pack() const {
            int64_t h = std::hash<std::string>{}(shaderUUID) & ((1ULL << 48) - 1);
            return  ((h & ((1ULL << 48) - 1)) << 14)
                | ((static_cast<uint64_t>(blending) & 1ULL) << 13)
                | ((static_cast<uint64_t>(depthTest) & 1ULL) << 12)
                | ((static_cast<uint64_t>(cullMode) & 0x3FULL) << 6)
                | ((static_cast<uint64_t>(polygonMode) & 0x3FULL) << 0);
        }
